#!/usr/bin/env python
#
#  tests/test_pathway_equivalence.py
...
from crnverifier.utils import parse_crn, assign_crn_species, remove_species
from crnverifier.pathway_decomposition import (Path, 
                                               BasisFinderError, 
                                               NoFormalBasisError,
                                               get_formal_basis, 
                                               tidy,
...
class TestPathProperties(unittest.TestCase):
    ...
    def test_decomposable_final_states(self):
        ...
        path1 = Path(parse_crn(p1)[0], fs)
        path2 = Path(parse_crn(p2)[0], fs)
        path3 = Path(parse_crn(p3)[0], fs)
        path4 = Path(parse_crn(p4)[0], fs)
        path5 = Path(parse_crn(p5)[0], fs)
...
@unittest.skipIf(SKIP, "skipping tests")
class TestFormalBasis(unittest.TestCase):
    def test_no_basis_found(self):
        pf = 'A -> B; B -> C'
        pi = 'A <=> j; j -> i + C; C + i -> B; B -> C; i->j'
        p0 = 'A -> j; j -> i + C; C + i -> B'
        fcrn, fs = parse_crn(pf)
        icrn, _ = parse_crn(pi)
        with self.assertRaises(NoFormalBasisError) as e:
            basis_raw, basis_int = get_formal_basis(icrn, fs)

        cf = 'A + B <=> C + D'
        ci = 'A <=> i; i + B <=> j; j <=> k + C; k <=> D'
        fcrn, fs = parse_crn(cf)
        icrn, _ = parse_crn(ci)
        with self.assertRaises(NoFormalBasisError) as e:
            basis_raw, basis_int = get_formal_basis(icrn, fs)
    ...
    def test_STW2019_F3(self):
        ...
        crn2 = """
            ...
            m -> F
            """
        fcrn, fs = parse_crn(crn1)
        icrn, _ = parse_crn(crn2)

        basis_raw = [[['A', 'B'], ['D', 'E']], [['D'], ['F']]]
        basis_raw = sorted(sorted(rxn) for rxn in basis_raw)
        basis_int = []

        basis_raw1, basis_int1 = get_formal_basis(icrn, fs, modular = True)
        basis_raw1 = sorted(sorted(rxn) for rxn in basis_raw1)

        basis_raw2, basis_int2 = get_formal_basis(icrn, fs, modular = False)
    ...
    def test_STW2019_F4(self):
        ...
        fs2 = set(inter.keys()) 
        basis_raw1, basis_int1 = get_formal_basis(icrn, fs2, interpretation = None)

        assert sorted(braw1) == sorted(basis_raw1)
        assert sorted(bint1) == sorted(basis_int1)

        basis_raw2, basis_int2 = get_formal_basis(icrn, fs2, interpretation = inter)
    ...
    def test_STW2019_intro(self):
        ...
        crn1, fs = parse_crn(crn1)
        crn2, _ = parse_crn(crn2)
        crn3, _ = parse_crn(crn3)
        crn4, _ = parse_crn(crn4)
        crn5, _ = parse_crn(crn5)
        crn6, _ = parse_crn(crn6)

        basis1, _ = get_formal_basis(crn1, fs)
        basis1 = sorted(sorted(rxn) for rxn in basis1)

        with self.assertRaises(NoFormalBasisError) as e:
            get_formal_basis(crn2, fs)

        with self.assertRaises(NoFormalBasisError) as e:
            get_formal_basis(crn3, fs)

        with self.assertRaises(NoFormalBasisError) as e:
            get_formal_basis(crn4, fs)

        basis5, _ = get_formal_basis(crn5, fs)
        basis5 = sorted(sorted(rxn) for rxn in basis5)
        assert basis5 == basis1

        basis6, _ = get_formal_basis(crn6, fs)
    ...
    def test_crn_STW2019_text01(self):
        ...
        with self.assertRaises(NoFormalBasisError) as e:
            basis_raw, basis_int = get_formal_basis(icrn, fs1, interpretation = None)

        basis_raw, basis_int = get_formal_basis(icrn, fs2, interpretation = None)
        assert sorted(basis_raw) == icrn
        assert basis_int == []

        basis_raw, basis_int = get_formal_basis(icrn, fs1, interpretation = inter1)
        br = [[['A', 'B1'], ['C']], 
               [['A', 'B1', 'B2'], ['B2', 'C']], 
               [['A', 'B1', 'B2'], ['B1', 'C']], 
               [['A', 'B2'], ['C']]]
        ...
        basis_raw, basis_int = get_formal_basis(icrn, fs2, interpretation = inter2)
        assert sorted(basis_raw) == icrn
        assert sorted(basis_int) == fcrn
    ...
    def test_STW19_EX01(self):
        ...
        FB = [[['A'], ['B']], 
              ...
              [['A'], ['X', 'Y']], 
              [['A'], ['X']]]
        vcrn = remove_species(icrn, fuels)
        intermediates, wastes, _ = assign_crn_species(vcrn, signals)
        vcrn = remove_species(vcrn, wastes)
        basis_raw, basis_int = get_formal_basis(vcrn, signals)
    ...
    def test_STW19_EX02(self):
        ...
        vcrn = remove_species(icrn, wastes)
        basis_raw, basis_int = get_formal_basis(vcrn, signals)
        assert sorted(basis_raw) == sorted(FB)
    ...
    def test_crn_JDW2019_F5(self):
        ...
        with self.assertRaises(NoFormalBasisError) as e:
            basis_raw, basis_int = get_formal_basis(icrn, fs)
        with self.assertRaises(NoFormalBasisError) as e:
            basis_raw, basis_int = get_formal_basis(icrn, fs, interpretation = inter)
